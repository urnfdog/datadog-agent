...
func (d *DiscarderMonitor) SendStats() error {
	...
	for eventType, stats := range globalStats {
		...
		var tag string
		if eventType == 0 {
			tag = "discarder_type:pid"
		} else {
			tag = fmt.Sprintf("discarder_type:%s", eventType)
		}

		_ = d.client.Count(metrics.MetricDiscarderAdded, int64(stats.DiscardersAdded), []string{tag}, 1.0)
		_ = d.client.Count(metrics.MetricEventDiscarded, int64(stats.EventDiscarded), []string{tag}, 1.0)
